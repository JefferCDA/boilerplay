...
const { snakeToCamel } = require('../src/libraries/tools/datatype');
...
const createMigrations = (dbName) => {
  ...
  databaseTables.forEach((fill) => {
    ...
    const tableName = snakeToCamel(file);
    ...
    if (!modelMigrations.length) {
      ...
      + `const { modelSchema } = require('../../../src/models/${dbName}/${file}/${file}.model');\n`
        + `\n`
        + `module.exports = {\n`
        + `  up: (queryInterface, DataTypes) => {\r`
        + `    const ${tableName} = queryInterface.createTable('${tableName}', modelSchema(DataTypes), {\r`
        + `      timestamps      : false,\r`
        + `      freezeTableName : true,\r`
        + `      tableName       : '${tableName}',\r`
        + `    });\r`
        + `    return ${tableName};\r`
        + `  },\n\n`
        + `  down: (queryInterface) => {\r`
        + `    return queryInterface.dropTable('${tableName}');\r`
        + `  },\n`
        + `};\n`;
